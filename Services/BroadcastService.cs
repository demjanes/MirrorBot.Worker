...
namespace MirrorBot.Worker.Services
{
    public sealed class BroadcastService
    {
        ...
        public async Task SendToLastActiveBotAsync(
            string text,
            TimeSpan activeWithin,
            CancellationToken ct)
        {
            ...
            await foreach (var user in _users.StreamForBroadcastAsync(activeAfterUtc, ct))
            {
                ...
                try
                {
                    // В Bot API отправка идёт по chat_id, поэтому мы храним LastChatId [web:81]
                ...
                {
                    // типичный кейс: пользователь заблокировал бота [web:170]
